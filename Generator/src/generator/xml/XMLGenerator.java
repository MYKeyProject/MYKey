			...
			for (int row = 0; row < numOfRow; row++) {
				...
				for (int element = 0; element < rowLength; element++) {
					...
					for (int codeNum = 0; codeNum < keyCode.length; codeNum++) {
						shiftXMLFileWriter
								.append(Integer.toString(keyCode[codeNum]));
					...
					keyWidth = Double.parseDouble(String.format("%.2f",
							keyWidth));
					shiftXMLFileWriter.append("\" android:keyWidth=\"" + keyWidth
							+ "%p\" ");
					...
					keyHeight = Double.parseDouble(String.format("%.2f",
							keyHeight));
					shiftXMLFileWriter.append("android:keyHeight=\"" + keyHeight
							+ "%p\" ");
					...
					if (currentKey.getKeyEdgeFlags() == RIGHT) {
						shiftXMLFileWriter.append("android:keyEdgeFlags=\"right\" ");
					}

					if (currentKey.getKeyEdgeFlags() == LEFT) {
						shiftXMLFileWriter.append("android:keyEdgeFlags=\"left\" ");
					}
			...
			for (int row = 0; row < numOfRow; row++) {
				...
				for (int element = 0; element < rowLength; element++) {
					...
					if (isRowStart) {
						...
						notShiftXMLFileWriter.append("\t<Row android:keyHeight=\""
								+ rowHeight + "%p\">");
					...
					keyWidth = Double.parseDouble(String.format("%.2f",
							keyWidth));
					notShiftXMLFileWriter.append("\" android:keyWidth=\"" + keyWidth
							+ "%p\" ");
					...
					keyHeight = Double.parseDouble(String.format("%.2f",
							keyHeight));
					notShiftXMLFileWriter.append("android:keyHeight=\"" + keyHeight
							+ "%p\" ");
					...
					if (currentKey.getKeyEdgeFlags() == RIGHT) {
						notShiftXMLFileWriter.append("android:keyEdgeFlags=\"right\" ");
					}

					if (currentKey.getKeyEdgeFlags() == LEFT) {
						notShiftXMLFileWriter.append("android:keyEdgeFlags=\"left\" ");
					}
					...
					if (currentKey.getIsRepeatable() == TRUE) {
						notShiftXMLFileWriter.append("android:isRepeatable=\"true"
								+ "\" ");
					}
					notShiftXMLFileWriter.append(" />");
					notShiftXMLFileWriter.newLine();
